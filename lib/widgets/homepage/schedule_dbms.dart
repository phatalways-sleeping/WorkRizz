...
class ScheduleData {
  ...
  ScheduleData() {
    for (int i = 0; i < 24; i++) { // create 24 tasks hour
      taskname.add("Free time");
      taskColor.add(Colors.white);
  ...
  bool isSameTask(int firstID, int secondID){
    return taskname[firstID] == taskname[secondID] && taskDescription[firstID] == taskDescription[secondID];
  }
  ...
  List <String> getTaskDescription(){
    ...
    for(int i = 1; i < taskname.length; i++){
      if (!isSameTask(i - 1, i))
      returnList.add(taskDescription[i]);
    }
    return returnList;
  }
